...
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="stylechatinf.css"> 
...
$sql = "SELECT DISTINCT marque.name, marque.id 
FROM messages
JOIN marque  ON (messages.receiver_id = marque.id AND messages.receiver_type = 'marque')
    OR (messages.sender_id = marque.id AND messages.sender_type = 'marque')
WHERE messages.sender_type = 'influenceur'
    AND messages.sender_id = $user_id
    OR messages.receiver_type = 'influenceur'
